...
if(isset ($_SESSION["username"])) //call this function to check if session exists or not
{
	...
	$queryGe1 = "select * from users where username = '".$session."'";	
	$resultGe1 = mysqli_query($link,$queryGe1);
	if(!$resultGe1)
{	die ("Invalid Query - get Items List: ". mysqli_error($link));	}
	else{	
	$ro1= mysqli_fetch_array($resultGe1, MYSQLI_BOTH);
	$user = $ro1['uid'];
}
	?>
<!DOCTYPE html>
...
<body>
<?php
$queryGet = "select * from attendance where usrID = '$user'";	
		...
		<tr>
			...
			{	
				...
				{	die ("Invalid Query - get Items List: ". mysqli_error($link));	}
					...
					$resultGet1 = mysqli_query($link,"select * from class where cID = '".$row1['cID']."'");
